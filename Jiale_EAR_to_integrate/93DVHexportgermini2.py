...
def run_dvh_export_process(all_organized_files_by_patient, output_dir):
    ...
    calculates DVHs (multiplying dose by 100), and writes to patient-specific Excel files,
                    ...
                    try:
                        ...
                        for i in range(len(dvh_obj.bincenters)):
                            dvh_points_for_roi.append({
                                'Dose_Gy': round(dvh_obj.bincenters[i] / 100, 6), # Dose values multiplied by 100
        ...
        try:
            with pd.ExcelWriter(excel_path_for_patient, engine='openpyxl') as writer:
                ...
                for sheet_base_name_key in sorted_sheet_base_name_keys: 
                    ...
                    for roi_name_iter in rois_to_write_in_order_for_this_sheet:
                        dvh_points_list = rois_data_for_sheet.get(roi_name_iter, [])
                        doses = [p['Dose_Gy'] for p in dvh_points_list] # Key 'Dose_Gy' is still used
                        ...
                        sheet_data_dict_for_df[(roi_name_iter, 'Dose_Gy')] = doses # Column name 'Dose_Gy' is still used
                    ...
                    for roi_name_val in rois_to_write_in_order_for_this_sheet:
                        ordered_multi_index_tuples.append((roi_name_val, 'Dose_Gy')) # Column name 'Dose_Gy'
...
if __name__ == '__main__':
    # --- User Configuration for Output ---
    # Please update this path if needed before running the script, or it will use this default.
    output_export_directory = "/home/dro/Desktop/jiale/imaging_dosimetry (RDscaled)/VMAT/Test" # EXAMPLE: "C:/DVH_Exports"
    # -------------------------------------
    ...
    # Check if the output_export_directory is still the placeholder value and warn if so.
    # You might want to make this a stricter check or prompt for output_export_directory as well if it's "/path/to/your/output_folder"
    if output_export_directory == "/path/to/your/output_folder": # A generic placeholder often used in examples
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print("!!! WARNING: 'output_export_directory' might be set to a placeholder.        !!!")
        print(f"!!! Current output path: {output_export_directory}                                  !!!")
        print("!!! Please ensure this is the correct path or update it in the script.       !!!")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        # Optionally, add: exit(1) if you want to force the user to change it.
    ...
    print(f"Input DICOM folder: {dicom_input_folder}")
    print(f"Output Excel files will be saved to: {output_export_directory}")
